...
function downloadFiles(api, files, overwrite = false, outputStream = new ConsoleOutputStream()) {
    return new Promise(function(resolve, reject) {
        async.forEachSeries(files, async.ensureAsync(function(file, callback) {
            try {
                ...
                request.get({
                    url: `${api.m_address}?dl=${encodeURI(file.cloudflow)}&session=${api.m_session}`,
                    ...
                    callback(err);
                }).pipe(fs.createWriteStream(file.fs, {
                    autoClose: true,
                    flags: 'w'
                })).on('error', function(err) {
                    outputStream.writeLine(err);
