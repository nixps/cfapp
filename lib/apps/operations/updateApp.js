...
const checkFileStores = require('../files/checkFileStores');
...
async function updateApp(app, options, outputStream = new ConsoleOutputStream()) {
    ...
    // Checks if the app is compatible, throws if the application is not compatible
    await app.checkCompatibility(apiAsync, options.forceCloudflowVersion);
    ...
    // Check the license of the new version of the app
    const licenseOK = app.validateLicense(api)
    if (! licenseOK) {
        throw new Errors.MissingLicenseError(app.name, app.version, app.license);
    }

    //Check if filestores are available
    const missing_filestores = await checkFileStores(api, app.files);
    if(missing_filestores.length) {
        throw new Errors.MissingFilestoresError(missing_filestores);
    }
    ...
    const keep = _.uniq(
        _.concat(
            _.map(updateList, u => u.remoteFlow.name),
