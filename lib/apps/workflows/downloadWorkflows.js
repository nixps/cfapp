 ...
 */
async function downloadWorkflows(api, app, overwrite = false, outputStream = new ConsoleOutputStream()) {
    const remoteWorkflows = (await PromiseCloudflowAPI.whitepaperList(api)).results;
    ...
    for(const name of app.workflows) {
        ...
        const workflowJSON = await PromiseCloudflowAPI.whitepaperGet(api, remoteWorkflow._id);
        const workflowContents = JSON.stringify(workflowJSON, null, '\t');
