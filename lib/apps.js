...
const PromiseCloudflowAPI = require('../lib/apps/PromiseCloudflowAPI');
...
function uploadApp(app, options, outputStream = new ConsoleOutputStream()) {
    ...
    // Adding the files
    return uploadFiles(apiAsync, app.getLocalFiles(), parameters.overwrite, outputStream).then(function() {
...
function removeApp(appName, options, outputStream = new ConsoleOutputStream()) {
    ...
    return new Promise(function(resolve, reject) {
        async.parallelLimit(installedFiles.map(function(file) {
            return function(callback) {
                PromiseCloudflowAPI.assetListFile(apiAsync, file.cloudflow).then(function(assets) {
                    if (Array.isArray(assets) === false || assets.length === 0) {
                        outputStream.writeLine(`asset "${file.cloudflow}" does not exist anymore`);
                        return Promise.resolve(false);
                    }

                    outputStream.writeLine(`removing asset: ${file.cloudflow}`);
            ...
            if (error) {
                outputStream.writeLine(`Could not remove app: ${appName}`);
                reject(error);
            }
            else {
...
function updateApp(app, options, outputStream = new ConsoleOutputStream()) {
    ...
    return removeApp(app.name, parameters, outputStream).then(function() {
