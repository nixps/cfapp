...
function uploadApp(app, options) {
    ...
    // Adding the files
    return uploadFiles(api, app.getLocalFiles(), parameters.overwrite).then(function() {
...
function downloadApp(app, options) {
    ...
    // Getting the files
    return downloadFiles(api, app.getRemoteFiles(api), parameters.overwrite);
}
...
function updateApp(app, options) {
    ...
    if (canRegisterApps(api) === false) {
        console.log(`skipping update of ${app.name}`);
        console.log(`no support for application updates this Cloudflow build b${api.portal.version().build}`);
        return Promise.reject(new Error(`no support for application updates in build b${api.portal.version().build}`));
    }

    const appDefinition = getAppDefinition(api, app.name);

    if (semver.valid(appDefinition.version) === null && parameters.force === false) {
        throw new Error(`invalid version for REMOTE ${app.name}, force to update`);
    }

    if (semver.valid(app.version) === null) {
        throw new Error(`invalid version on LOCAL ${app}, specify a valid version to update`);
    }
