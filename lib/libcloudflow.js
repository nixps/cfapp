...
module.exports = {
    ...
    create_installer: function (argv) {
        ...
        fs.writeFileSync(scripts_folder + _os.slash + "postinstall",
            ...
            "ps aux | grep cloudflow_plug-in_suite//nucleusd | grep -v grep > /dev/null\n" +
            '[ "$?" -eq 0 ] && echo "Stopping cloudflow_plug-in_suite service ..." && sudo -u "$loggedInUser" ../node-*/bin/node bin/cfapp.js frame stop' + "\n" +
            ...
            '[ ! -z "$loggedInUser" ] && sudo -u "$loggedInUser" ../node-*/bin/node bin/cfapp.js frame start' + "\n"
        );
        ...
        var items = fs.readdirSync(argv.output_location + "installer/files/cloudflow_plug-in_suite/");
        ...
        let sign_files = [
            "nucleusd",
            "cfapp-" + build_version + "/mac/node-v7.8.0-darwin-x64/bin/node",
