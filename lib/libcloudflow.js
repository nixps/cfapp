 ...
 //       }
    ...
    create_installer: function (argv){
        ...
        fs.writeFileSync(files_folder + _config + _os.slash + "uninstall.command",
            '#!/bin/bash -x\n'+
            '/Applications/frame/cfapp-*/mac/node-*/bin/node /Applications/frame/cfapp-*/mac/cfapp-master/bin/cfapp.js frame stop\n' +
            'sudo /Applications/frame/cfapp-*/mac/node-*/bin/node /Applications/frame/cfapp-*/mac/cfapp-master/bin/cfapp.js frame uninstall\n' +
            'exit\n'
        );
    ...
    create_msi: function (argv){ 
        ...
        fs.writeFileSync(files_folder + _os.slash + "launch.bat",
            '@echo off\r\n'+
            'cd "%ProgramFiles%\\frame\\cfapp-*\\win\\node*\\"\r\n' +
            'set "nodefolder=%cd%"\r\n' +
            'cd "%ProgramFiles%\\frame\\cfapp-*\\win\\cfapp-*\\"\r\n' +
            'set "cfappmaster=%cd%"\r\n' +
            '"%nodefolder%\\node.exe" "%cfappmaster%\\bin\\cfapp.js" frame install "%ProgramFiles%\\frame\\\\" --nocopy\r\n' +
        ...
        /*
@echo off
cd "%ProgramFiles%\frame\
FOR /F "tokens=*" %%g IN ('dir /b cfapp-*') do (SET cfapp=%%g)

xcopy "%ProgramFiles%\frame\%cfapp%" "%TEMP%\%cfapp%\" /E /O /Y /R /C /Q
xcopy "%ProgramFiles%\frame\clean.bat" "%TEMP%\%cfapp%\" /E /O /Y /R /C /Q
call "%TEMP%\%cfapp%\clean.bat" "%cfapp%"
        */
        ...
        'xcopy "%ProgramFiles%\\frame\\%cfapp%" "%TEMP%\\%cfapp%\\" /E /O /Y /R /C /Q\r\n' +
        'xcopy "%ProgramFiles%\\frame\\cleanup.bat" "%TEMP%\\%cfapp%\\" /E /O /Y /R /C /Q\r\n' +
        ...
        'del C:\\Users\\Public\\Desktop\\frame.lnk\r\n'+
        'PAUSE\r\n'+
        'exit'
        );
