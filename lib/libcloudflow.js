...
module.exports = {
 ...
 //       }
        ...
        if (cloudflow.setup.run_as_service)
        {
            ...
            var command="sudo -u $(stat -f '%Su' $HOME) " + cloudflow.nucleusd + " --install" + cloudflow.setup.cmd;
            console.log(command);
    ...
    uninstall: function() {
        ...
        try
        {
            if (cloudflow.setup.run_as_service)
            {
                var command= "sudo -u $(stat -f '%Su' $HOME) " + cloudflow.nucleusd+" --uninstall";

                if(cloudflow.setup.is_frame === true){
                    command += " --frame";
                }
                console.log(command);
    ...
    create_installer: function (argv){
        ...
        fs.writeFileSync(scripts_folder+_os.slash+"postinstall",
            "#!/bin/bash -x\n"+
            'echo "$UID" > /tmp/postinstall-uid.txt\n' +
            'echo "$EUID"> /tmp/postinstall-euid.txt\n'+ 
            "cd /Applications/frame/cfapp-*/mac/cfapp-master\n"+
            "sudo -u $(stat -f '%Su' $HOME) ../node-*/bin/node bin/cfapp.js frame install /Applications/frame/ --nocopy; cp $HOME/Library/LaunchAgents/com.nixps.frame.plist /Library/LaunchAgents/com.nixps.frame.plist \n"
        );
