...
class OperationTimeout extends Error {
    constructor() {
        const errorCode = 'CFAPPERR500'
        super(`Timeout while waiting on operation completion (${errorCode})`);
...
/**
 * Thrown when Cloudflow is not ready to receive MARS requests after timeout
 */
class CloudfowReadyOperationTimeout extends Error {
    constructor() {
        const errorCode = 'CFAPPERR508'
        super(`Timeout while waiting on Cloudflow to be ready for MARS requests (${errorCode})`);
        this.errorCode = errorCode;
    }
}
