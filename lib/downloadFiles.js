 ...
 */
function convertPath(appRoot, cloudflowPath) {
...
function downloadFiles(api, parameters, files) {
    async.forEachSeries(files, function(file, callback) {
        ...
        var assetFilename = convertPath(parameters.app, file);
        ...
        downloadFile(parameters.host, api.m_session, convertPath(parameters.app, file), file, callback);
    });
}
